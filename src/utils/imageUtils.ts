...
export const getRandomImagePair = () => {
  const allGroups = Object.keys(backgroundImages) as Array<keyof typeof backgroundImages>;
  ...
  const primaryImage = primaryGroup[Math.floor(Math.random() * primaryGroup.length)];
  const secondaryImage = secondaryGroup[Math.floor(Math.random() * secondaryGroup.length)];
